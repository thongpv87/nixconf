{ pkgs, config, lib, ... }:
with lib;
let cfg = config.nixconf.old.applications.taskwarrior;
in {
  options.nixconf.old.applications.taskwarrior = {
    enable = mkOption {
  ...
  config = let path = config.programs.taskwarrior.dataLocation;
  in mkIf (config.nixconf.old.applications.enable && cfg.enable) {
    programs.taskwarrior = {
      enable = true;
      config = {
        taskd = mkIf (cfg.server.enable) {
